	...
	const setVariantColours = (variantType: string) => {
		switch (variantType) {
			...
			case ButtonProperties.VARIANT.secondary.name:
				...
				break;
			case ButtonProperties.VARIANT.v2Primary.name:
				if (isTransparent) {
					setBackGround('transparent');
					setTextColor(`${ButtonProperties.VARIANT.v2Primary.background}`);
					setBorderColor(`${ButtonProperties.VARIANT.v2Primary.background}`);
					setIconFill(`${ButtonProperties.VARIANT.v2Primary.background}`);
				} else {
					setBackGround(`${ButtonProperties.VARIANT.v2Primary.background}`);
					setTextColor('white');
				}
				setHover(`${ButtonProperties.VARIANT.v2Primary.hover}`);
				setDisabled(`${ButtonProperties.VARIANT.v2Primary.disabled}`);
				setFocused(`${ButtonProperties.VARIANT.v2Primary.focused}`);
